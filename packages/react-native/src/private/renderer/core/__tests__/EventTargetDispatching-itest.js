...
describe('EventTarget-based Event Dispatching', () => {
  ...
  (ReactNativeFeatureFlags.enableNativeEventTargetEventDispatching() &&
    ...
    () => {
      ...
      // Child handler fires
      expect(childHandler.mock.calls.length - childCallsBefore).toBeGreaterThan(
        0,
      );
  ...
  (ReactNativeFeatureFlags.enableNativeEventTargetEventDispatching() &&
    ...
    it('direct (non-bubbling) events do not reach the document element or document', () => {
      ...
      asEventTarget(root.document).addEventListener('layout', documentHandler);
  ...
  describe('direct events (onLayout)', () => {
    it('does not bubble onLayout to ancestor views via the onLayout prop', () => {
      ...
      expect(childSpy.mock.calls.length - childCallsBefore).toBeGreaterThan(0);
  ...
  (ReactNativeFeatureFlags.enableNativeEventTargetEventDispatching() &&
    ...
    it('restricts the event path to the target (AT_TARGET only, no ancestor capture listeners)', () => {
      ...
      expect(ancestorCapture.mock.calls.length - ancestorCaptureBefore).toBe(0);
    });
  });
});
